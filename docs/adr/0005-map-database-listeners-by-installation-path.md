...
- Database Provider 失败时保留已发现结果并产生 Scan Notice；没有结果时 Database Discovery State 为“发现状态未知”，没有精确监听匹配时聚合 Database Listening State 为“监听状态未知”。已发现 Database Installation 自身可精确确定的监听状态不受影响。
