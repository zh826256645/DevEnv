...
- “环境配置”中的 Git 卡片默认折叠；展开后显示 Git LFS、Default Git Identity、默认分支、User Excludes File、签名配置和 Credential Helper Chain，并复用路径复制、键盘、VoiceOver 与“减少动态效果”交互，不计算就绪度、健康分或配置完成度。
