...
- 使用原生侧边栏提供“总览”“项目”“运行”“本地服务”和“系统信息”页面，默认进入总览；Runtime 与数据库列表归入系统信息页，不提供尚未实现能力的空壳页面。
...
- 刷新失败时保留上一次成功结果并显示横幅，下一次成功后清除。系统信息页的数据库区块与本地服务页面显示最近一次成功动态刷新时间；总览的“最近扫描”仍表示完整 Environment Scan。
