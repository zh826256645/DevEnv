...
required_patterns = {
    ...
    "locked v0.2.0 metadata": r"EXPECTED_RELEASE_VERSION:\s*0\.2\.0[\s\S]*?EXPECTED_RELEASE_BUILD:\s*[\"']?6[\"']?[\s\S]*?RELEASE_VERSION[\s\S]*?EXPECTED_RELEASE_VERSION[\s\S]*?builds != \{expected_build\}",
