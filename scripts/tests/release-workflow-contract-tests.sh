...
required_patterns = {
    ...
    "locked v0.1.4 metadata": r"EXPECTED_RELEASE_VERSION:\s*0\.1\.4[\s\S]*?EXPECTED_RELEASE_BUILD:\s*[\"']?5[\"']?[\s\S]*?RELEASE_VERSION[\s\S]*?EXPECTED_RELEASE_VERSION[\s\S]*?builds != \{expected_build\}",
