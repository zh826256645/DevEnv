...
required_patterns = {
    ...
    "locked v0.2.1 metadata": r"EXPECTED_RELEASE_VERSION:\s*0\.2\.1[\s\S]*?EXPECTED_RELEASE_BUILD:\s*[\"']?7[\"']?[\s\S]*?RELEASE_VERSION[\s\S]*?EXPECTED_RELEASE_VERSION[\s\S]*?builds != \{expected_build\}",
